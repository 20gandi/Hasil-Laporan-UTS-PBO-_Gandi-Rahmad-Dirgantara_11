...
#define CGI_PATH "cgi_/cgi"         // Path ke program_cgi
...
void parse_request_line(char *request, char *method, char *uri, char *http_version, char *query_string, char *post_data) {
    ...
    if (query_start != NULL) {
        *query_start = '\0'; // Pisahkan query string dari URI
        strcpy(query_string, query_start + 1); // Salin query string
    } else {
        query_string[0] = '\0'; // Tidak ada query string
    }
    ...
    if (body_start != NULL) {
        ...
        strcpy(post_data, body_start); // Salin data POST dari body
    } else {
...
void get_mime_type(char *file, char *mime) {
    ...
    if (dot == NULL) 
        ...
        strcpy(mime, "image/ico");
    else 
        strcpy(mime, "text/html");
}

void handle_method(char **response, int *response_size, char *method, char *uri, char *http_version, char *query_string, char *post_data) {
    char fileURL[256];
    snprintf(fileURL, sizeof(fileURL), "%s%s", FOLDER_DOCUMENT, uri);
    
    FILE *file = fopen(fileURL, "rb");

    // Jika file resource tidak ditemukan, kirimkan status 404 ke browser
    if (!file) {
        char not_found[BUFFER_SIZE];
        snprintf(not_found, sizeof(not_found), "%s 404 Not Found\r\n"
                 "Content-Type: text/html\r\n"
                 "Content-Length: %lu\r\n"
                 "\r\n"
                 "<h1>404 Not Found</h1>", http_version, strlen("<h1>404 Not Found</h1>"));
        ...
        *response_size = strlen(not_found);
        ...
        if (extension && strcmp(extension, ".php") == 0) {
            ...
            snprintf(command, sizeof(command), "%s --target=%s --method=%s --data_query_string=\"%s\" --data_post=\"%s\"", CGI_PATH, fileURL, method, query_string, post_data);
            fp = popen(command, "r");
            ...
            snprintf(response_header, sizeof(response_header), "%s 200 OK\r\n"
                     "Content-Type: text/html\r\n"
            ...
            *response = (char *)malloc(*response_size + 1);
            ...
            snprintf(response_line, sizeof(response_line), "%s 200 OK\r\n"
            ...
            *response_size = fsize + strlen(response_line);
            ...
            // Alokasikan var msg_body untuk menampung response header dan isi file resource
...
int main() {
    ...
    while (1) {
        printf("Server menunggu request dari client...\n");
